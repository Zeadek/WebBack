...
func validate_data(formData FormData) (bool, FormErrors) {
	...
	if formData.Email == "" {
		flag = false
		data.Email = "Введите адрес электронной почты"
	} else if !re.MatchString(formData.Email) {
		flag = false
		data.Email = "Некоректная запись электронной почты, прим. adress@mail.domen"
	}

	re = regexp.MustCompile(`^\+\d{11}$`)
	if formData.Phone == "" {
		flag = false
		data.Phone = "Введите ваш номер телефона"
	} else if !re.MatchString(formData.Phone) {
		flag = false
		data.Phone = "Некоректная запись номера телефона, прим. +74957556983"
	}

	if len(formData.Langs) == 0 {
		flag = false
		data.Langs = "Выборите хотя бы 1 язык"
	}

	re = regexp.MustCompile(`^\d{4}(-\d{2}){2}$`)
	if !re.MatchString(formData.Dob) {
		flag = false
		data.Dob = "Введите дату рождения"
	}

	if formData.Bio == "" {
		flag = false
		data.Bio = "Запоните биографию"
	}

	if formData.Contract != "on" {
		flag = false
		data.Contract = "Подтвердите ознакомление с контрактом"
	}
